...
# 为协议封装引入“名称→SMILES→分子信息”的本地工具
from .py_smiles import (  # type: ignore
    chemical_name_to_molecular_formula,
    smiles_to_formula,
)
...
# ================== 4. 协议适配的薄封装函数（Reactivi_*）==================
# 这些函数名字对应 reactivityanalyzer_protocol.json 中的 function.name，
# 供 function-calling / tools 机制直接调用。

_GLOBAL_ANALYZER = ReactivityAnalyzer()


def Reactivi_get_compound_info(name: str):
    """
    协议封装：根据化合物名称获取信息。
    
    逻辑：
    1. 先查本地 ReactivityAnalyzer.compounds；
    2. 若本地不存在，则调用本地的名称解析工具：
       - chemical_name_to_molecular_formula(name)  → SMILES
       - smiles_to_formula(smiles)                 → 分子式/分子量等
    3. 用查到的信息构造一个标准化的 compound_data 返回：
       {
         \"name\": ...,
         \"smiles\": ...,
         \"source\": \"local_name_resolver\" 或 \"local\",
         \"formula_info\": {...}
       }
    """
    info = _GLOBAL_ANALYZER.get_compound_info(name)
    if info is not None:
        # 本地已有，包一层 source 标记
        enriched = {
            "name": name,
    ...
    # 本地没有：尝试使用 py_smiles 名称解析 → SMILES → 分子信息
    smiles = chemical_name_to_molecular_formula(name)
    if not smiles:
        return {
            "result": None,
            "metadata": {
                "source": "local_name_resolver",
                "step": "name_to_smiles",
                "error": f"cannot resolve name '{name}' to SMILES",
            },
        }

    formula_info = smiles_to_formula(smiles)
    ...
    return {"result": compound_data}


def _ensure_observed_dict(observed_phenomena):
    """
    工具内部小助手：既兼容 dict，也兼容 JSON 字符串。
    """
    if isinstance(observed_phenomena, dict):
        return observed_phenomena
    if isinstance(observed_phenomena, str):
        s = observed_phenomena.strip()
        if not s:
            return {}
        try:
            return json.loads(s)
        except Exception:
            # 如果不是 JSON，就简单返回一个带 raw_text 的占位
            return {"raw_text": observed_phenomena}
    return {}


def Reactivi_match_phenomenon(observed_phenomena):
    """
    协议封装：匹配观测到的现象与候选化合物的行为。
    observed_phenomena 可为 JSON 字符串或 dict。
    """
    obs = _ensure_observed_dict(observed_phenomena)
    scores = _GLOBAL_ANALYZER.match_phenomenon(obs)
    return {"result": scores}


def Reactivi_predict_compound(observed_phenomena):
    """
    协议封装：根据现象预测最可能的化合物。
    返回 predicted_compound、confidence、all_scores。
    """
    obs = _ensure_observed_dict(observed_phenomena)
    best, score, scores = _GLOBAL_ANALYZER.predict_compound(obs)
    result = {
        "predicted_compound": best,
        "confidence": score,
        "all_scores": scores,
    }
...
def Reactivi_solve_chemical_puzzle():
    """
    协议封装：调用原有的 solve_chemical_puzzle 工作流。
    """
    res = solve_chemical_puzzle()
    return {"result": res}


def Reactivi_visualize_analysis(result):
    """
    协议封装：可视化分析结果。
    返回一个占位结果（图像显示由 matplotlib 负责）。
    """
    fig = visualize_analysis(result)
    return {"result": "figure_generated"}
