...
zeopp_executable =  "YOUR PATH/zeopp-lsmo/zeo++/network" # The path to the zeo++ executable
models_directory = "YOUR PATH/Models/solvent_thermal_models/" # The location where we have the models
...
def generate_descriptors(cif_file_path: str):
    ...
    # Create 'primitive' and 'xyz' directories (if they don't exist)
    ...
    print(f"primitive_path:{primitive_path}")
...
def get_geometric_features(cif_file_path: str, geo_features_list):
    featurization_directory, cif_file = os.path.split(cif_file_path)
    featurization_directory = "TempFiles/adsorption_stability_temp_data/"
    # Create 'geometric' directory (if it doesn't exist)
...
def predict_stability(cif_file_name: str):
    ...
    # featurization_directory, cif_file_name = os.path.split(cif_file_path)
    featurization_directory = "TempFiles/adsorption_stability_temp_data"
    featurization_list = []
...
def predict_adsorption(cif_file_name: str):
    ...
    # base_path_RASPA = os.environ.get('CIF_FILE_PATH')
    base_path = Config().UPLOAD_FILES_BASE_PATH
    ...
    # featurization_directory, cif_file = os.path.split(cif_file_path)
    base_path_RASPA = "/home/hjj/project/"
...
#     os.environ['CIF_FILE_PATH'] = "ToolsAgent/TempFiles/cif/HKUST-1.cif"
#     cif_file_name = 'HKUST-1'

#     # output = predict_adsorption(cif_file_name)
#     output = predict_stability(cif_file_name)
